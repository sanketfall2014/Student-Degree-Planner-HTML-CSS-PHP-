...
//echo 'you are logged IN.<a href="logout.php" >Log out.</a><br>';
  ...
  $trid = gettrack($id);
							...
							//		echo "<script type='text/javascript'> alert('inside if');</script>";
									$query = "UPDATE usertrack SET track='".$track."', tid='d'  
							...
							//		echo "<script type='text/javascript'> alert('inside else');</script>";
										$query = "UPDATE usertrack SET track='".$track1."', tid='n'
										WHERE `username` ='". $_SESSION['user_id']."'"; 
										$result = mysql_query($query);
            ...
            <div class="row">
                <div class="col-md-4 col-sm-6 portfolio-item">
                    ...
                    <div class="portfolio-caption">
                        <h4>Core Courses</h4>
                        <p class="text-muted">Computer science: <?php echo gettrack($tr) ?> Track</p>
                    </div>
                </div>
                <div class="col-md-4 col-sm-6 portfolio-item">
            ...
            <div class="row">
                <div class="col-lg-12">
					...
					<?php
								...
								//	echo  "<script type='text/javascript'> alert('$trid');</script>";

									$sql = "SELECT cname, cnum, cpref, cdes, prereq, credits FROM course where tid = '".$trid."'";
									$result = $conn->query($sql);
									...
									<th>Credit Hours</th>
										 ...
										 while($row = $result->fetch_assoc()) {
											...
											//echo  "<script type='text/javascript'> alert('$bname $current_file');</script>";
											 echo "<tbody><tr><td>" . $row["cpref"]. "</td><td>" . $row["cnum"]. "</td><td>" . $row["cname"]. "</td><td>" . $row["prereq"]. "</td><td>" . $row["credits"]. "</td><td> <form action=\"". $current_file. "\" method=\"POST\">
											</td></tr></tbody>";
                ...
                <div class="col-lg-12">
                    <h2 class="section-heading">Available Timeslots</h2> <br><br>
								...
								<?php
									...
									<input type="submit" name="capt" class="buttons" value="Cancel an appointment"/> <br><br></form>
							  ...
							  <h3 class="section-heading">Advisor-2</h3>
								...
								<thead>
								  <tr>
									...
									$id = gettimefield($field1);
									$timeslot = gettimefield($field2);


									$sql = "Insert into advapt values('".$id."','".$timeslot."')";
									
									if ($query_run1 = mysql_query($sql)) {
									echo  "<script type='text/javascript'> alert('Appointment caanceled succesfully..');</script>";
										header('Location:user.php');
									}
									 else {
									echo  "<script type='text/javascript'> alert('You have already added timeslot');</script>";
									...
									$sql = "Delete from regtslot where username='".$_SESSION[user_id]."' and timeslots='".$timeslot."'";
									//	echo "</table>";
									if ($query_run1 = mysql_query($sql)) {
									echo  "<script type='text/javascript'> alert('Appointment made succesfully..');</script>";
										header('Location:user.php');
